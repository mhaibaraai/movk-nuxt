...
const { data: commits } = await useLazyFetch('/api/github/commits', {
  key: `commit-changelog-${name}`,
  query: {
    path: `src/${props.prefix ? `${props.prefix}/` : ''}${camelName}.ts`
  }
})

function normalizeCommitMessage(commit: { sha: string, message: string }) {
  const prefix = `[\`${commit.sha.slice(0, 5)}\`](https://github.com/mhaibaraai/movk-core/commit/${commit.sha})`
  const content = commit.message.replace(/\(.*?\)/, '')
    .replace(/#(\d+)/g, '<a href=\'https://github.com/mhaibaraai/movk-core/issues/$1\'>#$1</a>')
