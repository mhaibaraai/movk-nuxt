...
::field-group
  ...
  Default font; must match the `name` of one entry in `fonts`. When omitted, `--font-sans` is not injected and the font is controlled entirely by your project's CSS `@theme`. For example `'Alibaba PuHuiTi'`.
  ::

  ::field{name="fonts" type="ThemeFontConfig[]"}
  `ThemePicker` font options; when provided, completely replaces the built-in list. Each item is `{ name, href? }`.
    ...
    ::collapsible
      ...
      ::field-group
        ...
        Full URL of the font CSS file. For example `'https://cdn.mhaibaraai.cn/fonts/alibaba-puhuiti.css'`. When omitted, the font is assumed to be hosted on Google Fonts and the URL is derived from `name`. Fonts not registered in `fonts` never trigger a request.
        ::
      ::
    ::
  ::
