...
::note
Precedence: request-level `show` > request-level `successMessage` / `errorMessage` > global `success.show` / `error.show` / `methods` > global `enabled`. With `toast.success.show: false` set globally, both `{ success: { show: true } }` and `{ successMessage: '...' }` still show the toast; pass `{ success: false }` or `{ success: { show: false } }` to stay silent.
::

::tip{icon="i-lucide-arrow-right" to="/docs/getting-started/configuration#toast-options"}
To notify only on POST / PUT / PATCH / DELETE, set `toast.success.methods` in the global or endpoint config — the field is not accepted per request
::
