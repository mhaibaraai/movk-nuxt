...
└── useApiStream          $api.stream() + SSE parsing + abort
```

Streaming responses (`text/event-stream`) never enter the business-check and unwrapping chain: as soon as
`onResponse` detects a readable stream it short-circuits, handing over to
[`useApiStream`](/en/docs/composables/use-api-stream) or [`createChatTransport`](/en/docs/api/chat-transport).
