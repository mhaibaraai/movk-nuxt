...
seo:
  ...
  description: Organize a Movk Nuxt project by business domain — shared types, per-module API layers and the Nuxt 4 shared/ directory for full-stack type reuse.
---
...
业务代码应尽量**从 `@movk/nuxt` 派生类型**，而非重复声明，以保证与组件/composable 的实际签名同步演进。

### 索引访问派生 prop 回调

需要为 `DataTable` 等组件的函数式 prop 抽出具名回调时，用索引访问从组件 props 类型派生，参数交由 contextual typing 推断，避免手写参数类型：

```ts
import type { DataTableColumn, DataTableDataColumn, DataTableProps } from '@movk/nuxt'

interface Person { id: string, name: string, salary: number }

// 从列类型派生 cell / truncate / tooltip 回调
const cell: DataTableDataColumn<Person>['cell'] = ({ row }) => row.original.name

// 从组件 props 派生 sortable / pinable / resizable 的函数形态
const sortable: DataTableProps<Person>['sortable'] = column => column.id !== 'id'
```

### 复用官方事件处理类型

事件处理器直接复用导出的处理器类型，而非自行拼装事件签名：

```ts
import type { DataTableSelectHandler } from '@movk/nuxt'

const onSelect: DataTableSelectHandler<Person> = (_event, row) => {
...
API 错误分类、请求选项等从 `@movk/nuxt` 导入，结合 `isBusinessError` 收窄：

```ts
import type { ApiError, UseApiFetchOptions } from '@movk/nuxt'

function isBusiness(error: ApiError | Error): error is ApiError {
  return (error as ApiError).isBusinessError === true
}
```

::tip{icon="i-lucide-shapes"}
独立工具函数等无法从组件/composable 派生的类型，统一从 `@movk/nuxt` re-export 入口导入；不要自定义与官方等价的类型。
::
