...
数据列（`DataTableDataColumn`）通过 `accessorKey` 绑定字段，常用属性：

- `header` 列头文本、`size` 列宽（数值或 `xs`/`sm`/`md`/`lg`/`xl` 预设）、`minSize`/`maxSize` 约束
- `align` 单元格对齐（`left`/`center`/`right`）
- `fixed` 固定到 `left`/`right`
- `cell` 自定义单元格渲染，`string` 直接显示，函数接收 `CellContext`
...
::component-example
---
name: 'data-table-sortable-example'
---
::
...
::component-example
---
name: 'data-table-pinable-example'
---
::
...
::component-example
---
name: 'data-table-resizable-example'
---
::
...
::component-example
---
name: 'data-table-truncate-example'
---
::
...
::component-example
---
name: 'data-table-visibility-example'
---
::
...
::component-example
---
name: 'data-table-functional-example'
---
::
