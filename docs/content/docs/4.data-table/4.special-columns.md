...
`selection`、`index`、`row-pinning`、`actions` 列与数据列一样支持 `visibility`、`pinable`、`resizable`、`size`、`minSize`、`maxSize`、`align`、`fixed`。特殊列默认固定宽 60；给 `minSize` / `maxSize`（且不设 `size`）即改为按内容自适应——`minSize` 为下限、`maxSize` 为拖拽上限。右侧唯一的操作列（`fixed: 'right'`）设 `minSize` 可让其随按钮数撑开，详见[数据列的列宽规则](/docs/data-table/columns)。
