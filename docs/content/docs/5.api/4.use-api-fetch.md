...
// 自定义文案同样是开启意图：全局关闭或方法未命中白名单时也会弹出
await useApiFetch('/users', { toast: { successMessage: '创建成功!' } })
```

::note
开关优先级：请求级 `show` 显式声明 > 请求级 `successMessage` / `errorMessage` > 全局 `success.show` / `error.show` / `methods` > 全局 `enabled`。全局配置为 `toast.success.show: false` 时，请求级传 `{ success: { show: true } }` 或 `{ successMessage: '...' }` 都会弹出；要明确静默请传 `{ success: false }` 或 `{ success: { show: false } }`。
::

::tip{icon="i-lucide-arrow-right" to="/docs/getting-started/configuration#toast-选项"}
只想为 POST / PUT / PATCH / DELETE 弹成功提示，请在全局或端点配置 `toast.success.methods`；该字段不接受请求级传入
::
  ...
  **API 集成选项**
  ...
  Toast 提示配置。设为 `false` 禁用 Toast。
          ...
          ::collapsible
            ...
            ::field-group
              ::field{name="success" type="Partial<Toast> & { show?: boolean } | false"}
  成功提示配置。可以是完整的 Toast 属性对象，或 `false` 禁用成功提示；`show: true` 可覆盖全局关闭。
              ::

              ::field{name="error" type="Partial<Toast> & { show?: boolean } | false"}
  错误提示配置。可以是完整的 Toast 属性对象，或 `false` 禁用错误提示；`show: true` 可覆盖全局关闭。
              ::
