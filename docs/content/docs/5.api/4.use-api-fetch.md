...
Movk Nuxt 提供三种数据请求 composable，分别对应 SSR、延迟与纯客户端三种执行时机；签名一致，按场景选择即可。
...
::component-example
---
name: 'api-quickstart-ssr-example'
---
::

### 延迟请求

`useLazyApiFetch` 不阻塞路由切换，先完成页面渲染再进入 pending：

::component-example
---
name: 'api-quickstart-lazy-example'
---
::

### 纯客户端请求

`useClientApiFetch` 跳过 SSR，浏览器挂载后再发起请求：

::component-example
---
name: 'api-quickstart-client-example'
---
::

## 数据解包

[`$api`](/docs/api/api-plugin) 会自动解包 API 响应中 `dataKey` 对应的字段（默认为 `data`），`useApiFetch` 接收到的已经是解包后的业务数据:
...
::note
如果响应中不存在 `dataKey` 字段，会回退返回完整响应对象。`dataKey` 字段名可通过[模块配置](/docs/getting-started/configuration#response)自定义。
::

## 业务状态码检查

`$api` 拦截器会自动检查业务状态码（由[模块配置](/docs/getting-started/configuration#response)的 `response` 选项控制）。当业务状态码不在 `successCodes` 中时，会抛出 `ApiError` 并显示 Toast:
...
HTTP 200 但 `code` 不在 `successCodes` 内：拦截器抛出 `ApiError`，`isBusinessError === true`。

::component-example
---
name: 'api-errors-business-example'
---
::

### HTTP 400

后端 `createError({ statusCode: 400 })`：原生 `FetchError`，`statusCode === 400`。

::component-example
---
name: 'api-errors-http-400-example'
---
::

### HTTP 422 校验

后端 `createError({ statusCode: 422, data: { fields } })`：错误对象 `data` 字段携带服务端补充信息。

::component-example
---
name: 'api-errors-http-422-example'
---
::

### HTTP 500

服务器抛 500：原生 `FetchError`，`statusCode === 500`。

::component-example
---
name: 'api-errors-http-500-example'
---
::

### 网络中断

连接被强制断开：客户端拿到无 `statusCode` 的 `FetchError`。

::component-example
---
name: 'api-errors-network-example'
---
::
