...
└── useApiStream          $api.stream() + SSE 解析 + 中止
```

流式响应（`text/event-stream`）不进业务检查与解包链路：`onResponse` 识别到可读流即整段短路，
由 [`useApiStream`](/docs/composables/use-api-stream) 或 [`createChatTransport`](/docs/api/chat-transport) 接手。
...
`$api` 拦截器自动检查响应 `code` 是否在 `successCodes` 中，业务失败时抛出 `ApiError` 并显示 Toast。

::tip{icon="i-lucide-arrow-right" to="/docs/api/api-plugin#数据解包与业务校验"}
查看业务校验与 `skipBusinessCheck` 的交互示例
::

### 数据解包

`$api` 拦截器自动提取响应中 `dataKey`（默认 `data`）字段，泛型 `T` 直接对应业务数据类型。

::tip{icon="i-lucide-arrow-right" to="/docs/api/api-plugin#数据解包与业务校验"}
查看解包与 `skipUnwrap` 的交互示例
::
