...
默认情况下，`useApiFetch` 会检查业务状态码(通过模块配置的 `success` 选项):
...
`useApiFetch` 是对 Nuxt [`useFetch`](https://nuxt.com/docs/api/composables/use-fetch) 的封装，主要区别:

| 特性 | useFetch | useApiFetch |
|------|----------|-------------|
| 自动认证 | ❌ | ✅ 自动添加 token |
| 数据解包 | ❌ | ✅ 自动解包 `data` 字段 |
| 业务状态码检查 | ❌ | ✅ 自动检查并抛出错误 |
...
```ts
// ResT: API 响应 data 字段的原始类型
// DataT: transform 转换后的最终类型

// 示例 1: 无 transform,DataT 默认等于 ResT
const { data } = await useApiFetch<User[]>('/users')
// data: Ref<User[] | null>

// 示例 2: 使用 transform
const { data } = await useApiFetch<User[], string[]>('/users', {
...
`useApiFetch` 默认在服务端和客户端均执行:

```ts
// SSR: 服务端执行一次,客户端 hydrate
const { data } = await useApiFetch<User[]>('/users')
...
对于非 SEO 敏感数据，推荐使用 [`useClientApiFetch`](/docs/composables/use-client-api-fetch) 简化配置。

### 认证集成

`useApiFetch` 通过 `$api` 实例自动从 session 获取 token:

```ts
// 自动添加 Authorization 头
const { data } = await useApiFetch<User>('/user/profile')
