...
::warning
生产环境务必设置 `NUXT_SESSION_PASSWORD` 环境变量，用于加密 Session 数据。建议使用至少 32 位的随机字符串。
::

::tip{icon="i-lucide-link" to="https://github.com/atinux/nuxt-auth-utils?tab=readme-ov-file#configuration"}
了解更多 Session 配置选项，请参考 nuxt-auth-utils 文档
::
    ...
    ::collapsible
      **Parameters**
      ...
      ::field-group
        ...
        - 默认配置：`{ name: useSiteConfig().name, password: process.env.NUXT_SESSION_PASSWORD, cookie: { sameSite: 'lax' } }`
        ::
