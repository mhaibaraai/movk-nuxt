...
::tip{icon="i-lucide-settings" to="/docs/getting-started/configuration#认证配置"}
了解认证相关的全局配置选项
::
...
// 2. 带用户信息接口的登录
const handleLoginWithUserInfo = async () => {
  try {
    await login({
      loginPath: '/auth/login',
      credentials: loginForm.value,
      userInfoPath: '/auth/me'  // 登录后获取用户信息
    })
...
const handleAdvancedLogin = async () => {
  try {
    const result = await login({
      ...
      tokenExtractor: (res) => {
        // 从特殊字段提取
        return res.data?.accessToken || res.data?.jwt
      },
      // 自定义 session 构建
      sessionBuilder: (user, token) => ({
        user: {
...
const refreshToken = async () => {
  try {
    const { data } = await $api.$fetch('/auth/refresh', {
      method: 'POST'
    })
