...
```ts
const { exportCSS, exportConfig, resetTheme } = useTheme()

const css = exportCSS() // 当前主题的 CSS 变量字符串
const config = exportConfig() // app.config.ts 配置代码
resetTheme() // 重置回默认主题
```

`exportCSS()` 的结果可写入项目的 `main.css`，将运行时调试结果固化为默认主题（见[主题系统](/docs/getting-started/theme)）。
