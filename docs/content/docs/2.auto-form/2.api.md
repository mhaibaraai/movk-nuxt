...
::tip{icon="i-lucide-palette" to="https://ui.nuxt.com/docs/getting-started/theme/design-system#runtime-configuration"}

通过 `app.config.ts` 可以全局配置表单的样式，例如调整字段间距:

```ts [app.config.ts]
export default defineAppConfig({
  ui: {
    form: {
      base: 'space-y-4 min-w-md' // 设置表单字段间距和最小宽度
    },
    collapsible: {
      slots: {
        content: 'space-y-4' // 设置折叠面板内容的字段间距
      }
    }
  }
})
...
这样可以避免在每个 `AutoForm` 组件上重复设置 `class` 属性。
::
