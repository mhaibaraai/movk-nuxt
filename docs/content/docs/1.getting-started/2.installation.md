...
在 **Vue + Vite**（非 Nuxt）项目中使用？请看 Vue / Vite 模式说明。
::
...
::note
`@movk/nuxt` 内部已引入 `@nuxt/ui`，无需再单独 `@import "@nuxt/ui"`。整个项目应只有这一个 Tailwind 入口：多写一处 `@import "tailwindcss"` 会形成第二个 Tailwind 上下文，其默认主题变量会覆盖模块注入的 `--font-sans` 等值。
::
...
::note
`@nuxt/icon` 默认的 `globInclude` 不含 `.ts`，写在 composable、常量文件里的图标名会漏扫，故此处显式补上 `ts`。
::
