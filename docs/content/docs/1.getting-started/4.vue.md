...
`theme.colors`（颜色别名）、`theme.defaultVariants`（默认变体）、`theme.prefix`（与 Tailwind 前缀一致）、`theme.font`（字体）等，对齐 Nuxt 模式 `movk.theme`。`font` 与 Nuxt 模式行为一致：构建期把字体样式表的 `<link>` 注入 `index.html`（`vite dev`/`vite build` 均生效），并写入 `--font-sans`，详见 [主题系统 · 字体](/docs/getting-started/theme#字体)。
