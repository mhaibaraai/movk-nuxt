...
`icon.clientBundle` 控制 movk 组件与内置图标集用到的图标是否进入构建期图标包，对齐 Nuxt 模式 `movk.icon`。图标数据需要项目安装对应的 `@iconify-json/*` 包，未安装的图标集会被自动跳过（不会导致构建失败），详见 [主题系统 · 图标集](/docs/getting-started/theme#图标集)。
