...
主题模块统一管理颜色、圆角、字体与图标集，并向 `ThemePicker` 组件、`useTheme` composable 提供可选项。所有字段都在 `movk.theme` 下配置。
...
::field-group
  ...
  默认字体，取值需与 `fonts` 中某一项的 `name` 一致。缺省时不注入 `--font-sans`，字体由项目 CSS 的 `@theme` 决定。例如 `'Alibaba PuHuiTi'`。
  ::

  ::field{name="fonts" type="ThemeFontConfig[]"}
  `ThemePicker` 字体可选项；提供时完整替换内置列表。每项为 `{ name, href? }`。
    ...
    ::collapsible
      ...
      ::field-group
        ...
        字体名称，同时作为 `font-family` 与选择项标签。
        ::

        ::field{name="href" type="string"}
        字体 CSS 文件的完整 URL。例如 `'https://cdn.mhaibaraai.cn/fonts/alibaba-puhuiti.css'`。缺省时视为该字体托管在 Google Fonts，按 `name` 生成地址。未登记在 `fonts` 中的字体不会产生任何请求。
        ::
      ::
    ::
  ::
  ...
  movk: {
    theme: {
      fonts: [
        ...
        { name: 'Inter' } // 无 href，回退到 Google Fonts
      ]
    }
  }
})
...
::field-group
  ...
  成功提示配置。`show: false` 关闭成功提示。
  ::

  ::field{name="error" type="Partial<Toast> & { show?: boolean }"}
  错误提示配置。`show: false` 关闭错误提示。
  ::
::

::note
`enabled` 与 `show` 均为全局开关，请求级传 `toast: { success: { show: true } }` 可单次覆盖开启；优先级为请求级 `show` > 全局 `success.show` / `error.show` > 全局 `enabled`。
::
