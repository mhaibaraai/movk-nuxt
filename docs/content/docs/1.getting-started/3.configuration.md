...
::field-group
  ...
  `ThemePicker` 字体可选项；提供时完整替换内置列表。每项为 `{ name, href? }`，`href` 缺省时运行时回退到 Google Fonts。
    ...
    ::collapsible
      ...
      ::field-group
        ...
        字体 CSS 文件的完整 URL。例如 `'https://cdn.mhaibaraai.cn/fonts/alibaba-puhuiti.css'`。缺省时按 `name` 回退到 Google Fonts。
        ::
      ::
    ::
  ::
