...
export default defineNuxtConfig({
  ...
  llms: {
    ...
    full: {
      ...
      description: '基于 Nuxt UI 与 Zod v4 构建的 UI 工程套件，提供两种使用方式。完整能力面向 Nuxt 4：AutoForm（Schema 驱动的自动表单，多控件类型、自定义控件、布局与条件渲染）、DataTable（基于 TanStack Table，覆盖数据列/特殊列/树形数据/行交互/外观定制/分页与加载更多）、API System（useApiFetch/useLazyApiFetch/useClientApiFetch 多端点·自动认证·业务状态码检查·数据解包·Toast；useUploadWithProgress/useDownloadWithProgress 进度监控）、Standalone Components（DatePicker、PillGroup、SearchForm、Tree、WithCopy、ThemePicker 等）、Composables（useDateFormatter、useTheme、useMessageBox、useAutoForm 等）。其中 UI 层——独立组件、AutoForm、DataTable、主题与非服务端 Composables——另可经 @movk/nuxt/vite + @movk/nuxt/vue-plugin 在纯 Vue + Vite 项目中直接使用；API 集成域依赖 Nuxt 服务端运行时，仅 Nuxt 模式可用。'
    },
