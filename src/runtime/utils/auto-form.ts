 ...
 * @returns 默认值
 */
export function generateDefaultValue(schema: z.ZodType): any {
  // 首先检查是否有默认值定义
  ...
  // 提取核心 schema（去除 optional, nullable 等装饰器）
  const { coreSchema } = extractDecorators(schema)
  const typeName = (coreSchema as any)?._def?.typeName || (coreSchema as any)?.type
  ...
  switch (typeName) {
    ...
    case 'object': {
      ...
      for (const [key, fieldSchema] of Object.entries(shape)) {
        result[key] = generateDefaultValue(fieldSchema as z.ZodType)
      }
      return result
    }
    default:
