...
function extractSchemaInfo(schema: z.ZodType, globalMeta: GlobalMeta, autoGeneratedLabel: string) {
  ...
  // 3. 检查是否为布局分组
  const isLayoutGroup = customMeta?.__isLayoutGroup__ === true
  const finalType = isLayoutGroup
    ? 'layoutGroup'
    : (customMeta?.component ? undefined : (customMeta?.type ?? coreSchema?.type))
  ...
  return { decorators, mergedMeta, coreSchema, isLayoutGroup }
}
...
export function introspectSchema(
  ...
  const { decorators, mergedMeta, coreSchema, isLayoutGroup } = extractSchemaInfo(schema, globalMeta, autoGeneratedLabel)
  const finalType = mergedMeta.type

  // 布局分组处理 - 特殊的对象类型，不产生 state 字段
  if (isLayoutGroup && finalType === 'layoutGroup' && coreSchema) {
    return handleLayoutGroupField(coreSchema, mapping, parentPath, globalMeta, decorators, mergedMeta)
  }
...
/**
 * 布局分组字段处理逻辑
 * 布局分组是特殊的对象类型，不产生 state 字段，子字段路径直接展平到父级
 */
function handleLayoutGroupField(
  coreSchema: z.ZodType,
  mapping: AutoFormControls,
  parentPath: string,
  globalMeta: GlobalMeta,
  ...
  if (!parentPath) {
    for (const key of shapeKeys) {
      if (key in shape) {
        const child = shape[key] as z.ZodType
        field.children.push(...introspectSchema(child, mapping, key, globalMeta))
      }
    }
  } else {
    // 嵌套布局分组：子字段路径使用父路径作为前缀（不包含当前分组节点）
    for (const key of shapeKeys) {
      if (key in shape) {
        const child = shape[key] as z.ZodType
        const pathSegments = toPath(parentPath)
