...
//   pinable?: boolean
//   indentSize: number
//   /** 全局数据列溢出 Tooltip，列级 tooltip 可覆盖 */
//   tooltip?: boolean | number
//   /** 全局 Tooltip 透传 props，列级 tooltipProps 可覆盖 */
//   tooltipProps?: DataTableDataColumn<unknown>['tooltipProps']
...
//   const needCustomCell = !!(col.formatter || effectiveTooltip || col.emptyText !== undefined || options.emptyCell !== false)

//   const def: ColumnDef<T, unknown> = {
//     accessorKey: id,
//     header: (effectiveSortable || effectivePinable)
//       ? (ctx: HeaderContext<T, unknown>) => renderHeaderActions(ctx, col.label ?? id, effectiveSortable, effectivePinable)
//       : (col.label ?? id),
...
//         const emptyText = col.emptyText !== undefined ? col.emptyText : options.emptyCell
